...
/* osfweb version: 16.0.16114.10000 */
/* office-js-api version: 20230118.4 */
/*
...
(function (OfficeExt) {
    var SovereignHelper = (function () {
        ...
        SovereignHelper.prototype.getEnvironmentName = function () {
            return this.getConfig().then(function (config) {
                return (config && config.envName) ? config.envName : null;
            });
        };
        SovereignHelper.prototype.getEnvironmentType = function () {
            return this.getConfig().then(function (config) {
                return (config && config.envType) ? config.envType : null;
            });
        };
        SovereignHelper.prototype.getTrustedHostOrigins = function () {
            return this.getConfig().then(function (config) {
                return config != null ? config.trustedHostOrigins : null;
            });
        };
        ...
        SovereignHelper.prototype.isSovereignHostTrusted = function (hostName) {
            if (!hostName) {
                return Office.Promise.resolve(false);
            }
            else if (hostName === window.location.hostname) {
                return Office.Promise.resolve(true);
            }
            return this.getTrustedHostOrigins().then(function (trustedHostOrigins) {
                if (!trustedHostOrigins || trustedHostOrigins.length == 0) {
                    return false;
                }
                var regexHostName = new RegExp(trustedHostOrigins.join("|"));
                return regexHostName.test(hostName);
            });
        };
        SovereignHelper.prototype.getConfigPath = function () {
            var basePath = OSF._OfficeAppFactory
                .getLoadScriptHelper()
                .getOfficeJsBasePath();
            if (basePath) {
                ...
                if (indexofRoot > -1) {
                    return (basePath
                        .toLowerCase()
                        .substring(0, indexofRoot + configRoot.length) + "officeconfig.json");
                }
            }
            return "";
        };
        ...
        SovereignHelper.prototype.tryLoadConfig = function () {
            ...
            if (configPath) {
                var _localStorage_1 = OSF.OUtil.getLocalStorage();
                var cacheKey_1 = configPath + "_cache";
                var cacheValue = _localStorage_1.getItem(cacheKey_1);
                var cacheResult_1 = null;
                if (cacheValue) {
                    try {
                        cacheResult_1 = JSON.parse(cacheValue);
                    }
                    catch (_a) { }
                    var maxAgeInMs = 1000 * 60 * 60 * 24;
                    var currentTime = new Date().getTime();
                    if (cacheResult_1 &&
                        cacheResult_1.config &&
                        cacheResult_1.createTime <= currentTime &&
                        cacheResult_1.createTime + maxAgeInMs >= currentTime) {
                        return Office.Promise.resolve(cacheResult_1.config);
                    }
                }
                return this.downloadConfig(configPath).then(function (config) {
                    if (config) {
                        cacheResult_1 = { createTime: new Date().getTime(), config: config };
                        _localStorage_1.setItem(cacheKey_1, JSON.stringify(cacheResult_1));
                    }
                    else if (cacheResult_1) {
                        config = cacheResult_1.config;
                    }
                    return config;
                });
            }
            return Office.Promise.resolve(null);
        };
        SovereignHelper.prototype.downloadConfig = function (configUrl) {
            return new Office.Promise(function (resolve, reject) {
                OSF.OUtil.xhrGet(configUrl, function (content) {
                    try {
    ...
    OfficeExt.SovereignHelper = SovereignHelper;
})(OfficeExt || (OfficeExt = {}));
...
OSF.InitializationHelper.prototype.performUserTrustCheck = function OSF_InitializationHelper$performUserTrustCheck(hostName, isHttps, funcToRun) {
    var me = this;
    var notHttpsErrorMessage = "NotHttps";
    var loadAgaveErrorUX = function () {
        ...
        if (officejsCDNHost && officejsCDNHost[0]) {
            ...
            var hashObj = {
                ...
                noHttps: !isHttps,
                validate: false,
                useHtmlPopup: OSF.OUtil.checkFlight(OSF.FlightTreatmentNames.HostTrustDialog),
                isConfirm: false
            };
                ...
                if ((event.source == hostUserTrustIframe.contentWindow) &&
                    ...
                    try {
                        ...
                        if (receivedObj.hostUserTrusted === true) {
                            ...
                            if (OSF.OUtil.checkFlight(OSF.FlightTreatmentNames.HostTrustDialog)) {
                        ...
                        document.body.textContent = Strings.OfficeOM.L_NotTrustedWAC;
                    }
                }
            }
            ;
...
OSF.InitializationHelper.prototype.checkWellKnownOrigins = function OSF_InitializationHelper$checkWellKnownOrigins(hostName, isHttps, onOriginCheckComplete) {
    var me = this;
    if (me._webAppState.clientEndPoint._hostTrustCheckStatus === Microsoft.Office.Common.HostTrustStatus.unknown) {
        ...
        if (me._webAppState.clientEndPoint._hostTrustCheckStatus != Microsoft.Office.Common.HostTrustStatus.nothttps) {
            if (OfficeExt.SovereignHelper.isSovereignVersion()) {
                if (!me._sovereignHelper) {
                    me._sovereignHelper = new OfficeExt.SovereignHelper();
                }
                var sovereignHelper = me._sovereignHelper;
                sovereignHelper.isConfigAvailable().then(function (isConfigAvailable) {
                    if (isConfigAvailable) {
                        sovereignHelper.isSovereignHostTrusted(hostName).then(function (isOriginValid) {
                            if (isOriginValid) {
                                me._webAppState.clientEndPoint._hostTrustCheckStatus = Microsoft.Office.Common.HostTrustStatus.trusted;
                            }
                            onOriginCheckComplete();
                        });
                    }
                });
                return;
...
OSF.InitializationHelper.prototype.checkReceiverOriginAndRun = function OSF_InitializationHelper$checkReceiverOriginAndRun(funcToRun) {
    ...
    var hostName = parsedHostname.hostname;
    var isHttps = parsedHostname.protocol == "https:";
    me.checkWellKnownOrigins(hostName, isHttps, function () {
        if (!me._webAppState.clientEndPoint._checkStatusLogged && me._hostInfo != null && me._hostInfo !== undefined) {
            var isLocalStorageAvailable = OSF.OUtil.getLocalStorage().isLocalStorageAvailable();
            OSF.AppTelemetry.onCheckWACHost(me._webAppState.clientEndPoint._hostTrustCheckStatus, me._webAppState.id, me._hostInfo.hostType, me._hostInfo.hostPlatform, me._webAppState.clientEndPoint._targetUrl, isLocalStorageAvailable);
        ...
        if (me._webAppState.clientEndPoint._hostTrustCheckStatus === Microsoft.Office.Common.HostTrustStatus.trusted) {
            ...
            me.performUserTrustCheck(hostName, isHttps, funcToRun);
        }
    });
};
...
(function (OSFLog) {
    ...
    var CheckWACHostUsageData = (function (_super) {
        ...
        Object.defineProperty(CheckWACHostUsageData.prototype, "isLocalStorageAvailable", {
            get: function () { return this.Fields["isLocalStorageAvailable"]; },
            set: function (value) { this.Fields["isLocalStorageAvailable"] = value; },
            enumerable: true,
            configurable: true
        });
        CheckWACHostUsageData.prototype.SerializeFields = function () {
            ...
            this.SetSerializedField("isLocalStorageAvailable", this.isLocalStorageAvailable);
        };
...
(function (OSFAriaLogger) {
    ...
    var TelemetryEventCheckWACHost = { name: "CheckWACHost", enabled: true, critical: false, points: [
            ...
            { name: "isLocalStorageAvailable", type: "boolean" },
        ] };
    var TelemetryEvents = [
...
(function (OSFAppTelemetry) {
    ...
    function initialize(context) {
        ...
        appInfo.hostJSVersion = "16.0.16114.10000";
    ...
    function onCheckWACHost(isWacKnownHost, instanceId, hostType, hostPlatform, wacDomain, isLocalStorageAvailable) {
        ...
        data.isLocalStorageAvailable = isLocalStorageAvailable;
        data.wacDomain = "";
        (new AppLogger()).LogData(data);
    }
        ...
        (function (Dialog) {
            ...
            function createNotificationPanelForCrossZoneIssue(dialogCssManager, windowUrl) {
                ...
                if (Strings.OfficeOM.L_NewWindowCrossZone) {
                    var dialogNotificationText = Strings.OfficeOM.L_NewWindowCrossZone.replace(/{1+}/gm, OfficeExt.WACUtils.getDomainForUrl(windowUrl));
...
OSF.OUtil.augmentList(Microsoft.Office.WebExtension.EventType, { SelectedItemsChanged: "olkSelectedItemsChanged" });
OSF.OUtil.augmentList(OSF.DDA.EventDescriptors, { OlkSelectionData: "OlkSelectionData" });
OSF.DDA.OlkSelectedItemsChangedEventArgs = function OSF_DDA_OlkSelectedItemsChangedEventArgs(eventData) {
    OSF.OUtil.defineEnumerableProperties(this, {
        "type": {
...
OSF.OUtil.augmentList(Microsoft.Office.WebExtension.EventType, { SensitivityLabelChanged: "olkSensitivityLabelChanged" });
OSF.OUtil.augmentList(OSF.DDA.EventDescriptors, { OlkSensitivityLabelChangedData: "OlkSensitivityLabelChangedData" });
OSF.DDA.OlkSensitivityLabelChangedEventArgs = function OSF_DDA_OlkSensitivityLabelChangedEventArgs(eventData) {
    OSF.OUtil.defineEnumerableProperties(this, {
        "type": {
...
function initialize() {
  ...
  addErrorMessage(9057, "RoamingSettingsExceededSize", getString("l_RoamingSettingsExceededSize_Text"));
  addErrorMessage(9058, "NativeLabelingNotEnabled", getString("l_NativeLabelingNotEnabled_Text"));
  addErrorMessage(9059, "SensitivityUnableToSetParent", getString("l_SensitivityUnableToSetParent_Text"));
  addErrorMessage(9060, "UserHasNoLabelsToSet", getString("l_UserHasNoLabelsToSet_Text"));
  addErrorMessage(9061, "FailedToGetLabelsCatalog", getString("l_FailedToGetLabelsCatalog_Text"));
  addErrorMessage(9062, "FailedToGetLabel", getString("l_FailedToGetLabel_Text"));
  addErrorMessage(9063, "FailedToSetLabel", getString("l_FailedToSetLabel_Text"));
  isInitialized = true;
}
...
// CONCATENATED MODULE: ./src/methods/insertOnSend.ts
...
function insertOnSend(dispId, apiName, data, args) {
  checkPermissionsAndThrow(2, apiName);
  var commonParameters = parseCommonArgs(args, false, false);
  var parameters = {
    appendTxt: data
  };
  ...
  if (isNullOrUndefined(parameters.appendTxt)) {
    ...
    validateInsertOnSendBodyParameters(parameters);
  }
...
function appendOnSend(data) {
  ...
  insertOnSend(100, "body.appendOnSendAsync", data, args);
}
...
// CONCATENATED MODULE: ./src/methods/prependOnSend.ts


function prependOnSend(data) {
  var args = [];
  ...
  checkFeatureEnabledAndThrow(Features.prependOnSend, "prependOnSendAsync");
  insertOnSend(197, "body.prependOnSendAsync", data, args);
}
...
function getBodySurface(isCompose) {
  ...
  if (isCompose) {
    objectDefine(body, {
      ...
      prependOnSendAsync: prependOnSend
    });
  }
...
var NotificationsActionExecuteFunctionActionId = "executeFunction";
...
var validateNotificationMessages_OSF = __webpack_require__(0);
...
function validateActionsDefinitionActionsType(actionsDefinition) {
  ...
  var actionType = actionsDefinition.actionType;
  var NotificationsAllowedActionIds = [NotificationsActionShowTaskPaneActionId];

  if (validateNotificationMessages_OSF.AppName.OutlookWebApp === getAppName()) {
    NotificationsAllowedActionIds.push(NotificationsActionExecuteFunctionActionId);
  }

  if (NotificationsAllowedActionIds.indexOf(actionType) === -1) {
...
// CONCATENATED MODULE: ./src/validation/validateSensitivityLabel.ts


function validateSensitivityLabel(sensitivityLabel) {
  if (!sensitivityLabel) {
    throw createArgumentError("sensitivityLabel");
  }

  if (typeof sensitivityLabel != "string" && !sensitivityLabel.id) {
    throw createArgumentError("sensitivityLabel.id", undefined);
  }
  ...
  if (typeof sensitivityLabel != "string" && sensitivityLabel.id) {
    ...
    if (sensitivityLabel.children != null) {
      throw createArgumentError(undefined, getString("l_SensitivityUnableToSetParent_Text"));
    }
  }
}
// CONCATENATED MODULE: ./src/methods/setSensitivityLabel.ts





function setSensitivityLabel(sensitivityLabel) {
  var args = [];
  ...
  checkFeatureEnabledAndThrow(Features.sensitivityLabels, "setSensitivityLabelAsync");
  checkPermissionsAndThrow(2, "sensitivityLabel.setAsync");
  var commonParameters = parseCommonArgs(args, false, false);
  validateSensitivityLabel(sensitivityLabel);
  var sensitivityLabelID = getSensitivityLabelID(sensitivityLabel);
  var parameters = {
    sensitivityLabelID: sensitivityLabelID
  };
  standardInvokeHostMethod(200, commonParameters.asyncContext, commonParameters.callback, parameters, undefined);
}

function getSensitivityLabelID(sensitivityLabel) {
  if (typeof sensitivityLabel != "string" && sensitivityLabel.id !== undefined) {
    return sensitivityLabel.id;
  } else {
    return sensitivityLabel;
  }
}
// CONCATENATED MODULE: ./src/methods/getSensitivityLabel.ts
...
function getSensitivityLabel() {
  ...
  checkFeatureEnabledAndThrow(Features.sensitivityLabels, "getSensitivityLabelAsync");
  checkPermissionsAndThrow(2, "sensitivityLabel.getAsync");
  var commonParameters = parseCommonArgs(args, true, false);
  standardInvokeHostMethod(201, commonParameters.asyncContext, commonParameters.callback, undefined, undefined);
}
// CONCATENATED MODULE: ./src/api/getSensitivityLabelSurface.ts



function getSensitivityLabelSurface() {
  var sensitivityLabel = objectDefine({}, {
    getAsync: getSensitivityLabel,
    setAsync: setSensitivityLabel
  });
  return sensitivityLabel;
}
...
function getMessageCompose() {
  var messageCompose = objectDefine({}, {
    ...
    sensitivityLabel: getSensitivityLabelSurface()
  });
...
// CONCATENATED MODULE: ./src/methods/getSelectedItems.ts
...
function getSelectedItems() {
  ...
  checkPermissionsAndThrow(3, "getSelectedItemsAsync");
  checkFeatureEnabledAndThrow(Features.multiSelect, "getSelectedItemsAsync");
  var commonParameters = parseCommonArgs(args, true, false);
  standardInvokeHostMethod(196, commonParameters.asyncContext, commonParameters.callback, undefined, undefined);
}
...
var prepareApiSurface_createMailboxSurface = function createMailboxSurface(target) {
  objectDefine(target, {
    ...
    getSelectedItemsAsync: getSelectedItems,
...
// CONCATENATED MODULE: ./src/methods/getSensitivityLabelsCatalog.ts
...
function getSensitivityLabelsCatalog() {
  ...
  checkFeatureEnabledAndThrow(Features.sensitivityLabels, "getSensitivityLabelsCatalogAsync");
  checkPermissionsAndThrow(2, "sensitivityLabelsCatalog.getAsync");
  var commonParameters = parseCommonArgs(args, true, false);
  standardInvokeHostMethod(199, commonParameters.asyncContext, commonParameters.callback, undefined, undefined);
}
// CONCATENATED MODULE: ./src/methods/getSensitivityLabelsCatalogIsEnabled.ts
...
function getSensitivityLabelsCatalogIsEnabled() {
  ...
  checkFeatureEnabledAndThrow(Features.sensitivityLabels, "getSensitivityLabelsCatalogIsEnabledAsync");
  checkPermissionsAndThrow(2, "sensitivityLabelsCatalog.getIsEnabledAsync");
  var commonParameters = parseCommonArgs(args, true, false);
  standardInvokeHostMethod(202, commonParameters.asyncContext, commonParameters.callback, undefined, undefined);
}
// CONCATENATED MODULE: ./src/api/getSensitivityLabelsCatalogSurface.ts



function getSensitivityLabelsCatalogSurface() {
  return objectDefine({}, {
    getAsync: getSensitivityLabelsCatalog,
    getIsEnabledAsync: getSensitivityLabelsCatalogIsEnabled
  });
}
...
var OutlookAppOm_OutlookAppOm = function () {
  function OutlookAppOm(appContext, targetWindow, appReadyCallback) {
    ...
    this.onInitialDataResponse = function (resultCode, data) {
      ...
      var context = _this.officeAppContext;
      context.sensitivityLabelsCatalog = getSensitivityLabelsCatalogSurface;
      setTimeout(function () {
        return _this.appReadyCallback();
      }, 0);
    };
...
function saveSettingsRequest(data) {
  ...
  if (JSON.stringify(serializedSettings).length > settingsMaxNumberOfCharacters) {
    var asyncResult_1 = createAsyncResult(undefined, saveSettingsRequest_OSF.DDA.AsyncResultEnum.ErrorCode.Failed, 9057, commonParameters.asyncContext, "");
        ...
        OSF.DDA.DispIdHost.addEventSupport(
            ...
            new OSF.EventDispatch(
                [
                    ...
                    Microsoft.Office.WebExtension.EventType.SelectedItemsChanged
                ]
            )
        );
    }
