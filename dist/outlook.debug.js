    ...
    (function (WebAuth) {
        ...
        function loadForAuthCode() {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (WebAuth.config.msal) {
                    OSF.OUtil.loadScript(WebAuth.config.msal, function (successOrEvent) {
                        var waited = 0;
                        var interval = 500;
                        if (typeof successOrEvent === 'boolean' && !successOrEvent) {
                            reject(successOrEvent);
                        ...
                        if (WebAuth.config.delayMsal && !validMsal()) {
                            var waitMsal_1 = setInterval(function () {
                                waited += interval;
                                if (validMsal() || waited > WebAuth.config.delayMsal) {
                                    clearInterval(waitMsal_1);
                                }
                            }, interval);
                        }
                    });
                }
                var validBrowserAuth = function () {
                    try {
                        return (BrowserAuth !== undefined && !!BrowserAuth);
                    }
                    catch (error) {
                        return false;
                    }
                };
                var runBrowserAuth = function (funcResolve, funcReject) {
                    if (!validBrowserAuth()) {
                        funcReject(new Error("Missing BrowserAuth namespace, BrowserAuth is not loaded."));
                    }
                    else {
                        if (WebAuth.config) {
                            BrowserAuth.Load(WebAuth.config, OSF._OfficeAppFactory.getHostInfo().osfControlAppCorrelationId).then(function (result) { funcResolve(result); }, function (result) { funcReject(result); });
                        }
                        else {
                            BrowserAuth.GetAuthConfig().then(function (configParent) {
                                WebAuth.config = configParent;
                                BrowserAuth.Load(WebAuth.config, OSF._OfficeAppFactory.getHostInfo().osfControlAppCorrelationId).then(function (result) { funcResolve(result); }, function (result) { funcReject(result); });
                            }, function () { funcReject(null); });
                        }
                    }
                };
                OSF.OUtil.loadScript(browserAuth_cdn_path, function (successOrEvent) {
                    ...
                    if (WebAuth.config.delayWebAuth && !validBrowserAuth()) {
                        var waitWebAuth_1 = setInterval(function () {
                            waited += interval;
                            if (validBrowserAuth() || waited > WebAuth.config.delayWebAuth) {
                                runBrowserAuth(resolve, reject);
                                clearInterval(waitWebAuth_1);
                            }
                        }, interval);
                    }
                    else {
