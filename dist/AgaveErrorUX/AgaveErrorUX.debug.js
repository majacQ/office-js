...
OSF.FlightTreatmentNames = {
    ...
    OSFEntitlementPropertyCleanup: "Microsoft.Office.SharedOnline.OSFEntitlementPropertyCleanup",
...
function _confirmHost(hostname, noHttps, useHtmlPopup, callback) {
    ...
    switch (+hostTrustResult) {
        case TrustResult.Unknown:
            ...
            if (useHtmlPopup) {
                ...
                var confirmDialogURL = "./index.html#" + encodeURIComponent(JSON.stringify(hashObj));
                var confirmWindow_1 = open(confirmDialogURL, "ConfirmTrust", "popup=yes,location=no,menubar=no,resizable=no,scrollbars=no,status=no,titlebar=no,toolbar=no,width=600,height=350,top=300,left=600");
                confirmWindow_1.addEventListener("load", function () {
                    confirmWindow_1._confirmMessageHostName(function (confirmed) {
                        value.Result = confirmed ? TrustResult.Trusted : TrustResult.Unknown;
                        _localStorage.setItem(keyResult, JSON.stringify(value));
                        _localStorage.removeItem(keyResultTimeStamp);
                        callback(confirmed);
...
function _showInfoBar(hostname, noHttps, useHtmlPopup, errorMessage, errorTitle) {
    ...
    continueButton.onclick = function (event) {
        _confirmHost(hostname, noHttps, useHtmlPopup, function (hostConfirmed) {
            if (hostConfirmed) {
...
try {
    ...
    var e = Function._validateParams(urlParams_1, [{ name: "error", type: String, mayBeNull: false },
        ...
        { name: "useHtmlPopup", type: Boolean, mayBeNull: true },
        { name: "isConfirm", type: Boolean, mayBeNull: true }
    ]);
    if (!e) {
        var locale_1 = window.navigator.language;
        if (urlParams_1.useHtmlPopup && !urlParams_1.validate) {
            if (OSF.isRTLLocale(OSF.getSupportedLocale(locale_1))) {
                document.getElementById("container").style.direction = 'rtl';
            }
        }
        if (urlParams_1.isConfirm && urlParams_1.useHtmlPopup) {
        }
        else if (urlParams_1.validate) {
            function validateHostnameTrust(event) {
                ...
                if (!e) {
                    ...
                    if (urlParams_1.useHtmlPopup) {
                        var postingObj_1 = { hostUserTrusted: hostUserTrusted };
                        event.source.postMessage(JSON.stringify(postingObj_1), event.origin);
                    }
                    else {
                        if (!hostUserTrusted) {
                            _loadStrings(locale_1, function () {
                                _confirmHost(receivedObj.hostname, receivedObj.noHttps, false, function (hostConfirmed) {
            ...
            _loadStrings(locale_1, function () {
                if (SupportedError[urlParams_1.error])
                    _showInfoBar(urlParams_1.hostname, urlParams_1.noHttps, urlParams_1.useHtmlPopup, Strings.OfficeOM["L_" + urlParams_1.error]);
            });
        }
    }
}
...
function _isRTLLocale() {
    return document.getElementById("container").style.direction === 'rtl';
}
function _confirmMessageHostName(callback, hostname, noHttps) {
    if (self !== top) {
        return;
    }
    var locale = navigator.language;
    _loadStrings(locale, function () {
        var rootDiv = document.getElementById("container");
        var contentContainer = document.createElement("div");
        if (_isRTLLocale()) {
            contentContainer.setAttribute("class", "confirmContainerRTL");
        }
        else {
            contentContainer.setAttribute("class", "confirmContainer");
        }
        rootDiv.appendChild(contentContainer);
        document.title = Strings.OfficeOM["L_ConfirmDialogConsentTitle"];
        var consentTitle = document.createElement("h1");
        consentTitle.setAttribute("class", "consentTitle");
        consentTitle.textContent = Strings.OfficeOM["L_ConfirmDialogConsentTitle"];
        contentContainer.appendChild(consentTitle);
        var message = document.createElement("div");
        message.setAttribute("class", "consentMessage");
        contentContainer.appendChild(message);
        var confirmDialogSpan = document.createElement("span");
        confirmDialogSpan.textContent = Strings.OfficeOM["L_ConfirmDialog"].replace('{0}', hostname);
        message.appendChild(confirmDialogSpan);
        if (noHttps == true) {
            var httpsMessageSpan = document.createElement("span");
            httpsMessageSpan.textContent = Strings.OfficeOM["L_NoHttpsWAC"];
            message.appendChild(httpsMessageSpan);
        }
        message.appendChild(document.createElement("br"));
        message.appendChild(document.createElement("br"));
        var confirmDialogConsentSpan = document.createElement("span");
        confirmDialogConsentSpan.textContent = Strings.OfficeOM["L_ConfirmDialogConsent"].replace('{0}', hostname);
        message.appendChild(confirmDialogConsentSpan);
        var footer = document.createElement("div");
        footer.setAttribute("class", "confirmFooter");
        contentContainer.appendChild(footer);
        var buttonOK = document.createElement("button");
        buttonOK.textContent = Strings.OfficeOM["L_OKButton"];
        buttonOK.setAttribute("class", "confirmOK");
        ...
        buttonCancel.textContent = Strings.OfficeOM["L_CancelButton"];
        buttonCancel.setAttribute("class", "confirmCancel");
